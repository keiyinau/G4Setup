...
void MyDetectorConstruction::ConstructSDandField() {
	...
	//if (logicDetector_pre!=NULL)
	//	logicDetector_pre->SetSensitiveDetector(sensDet);
	for(int i=0;i<23;i++){
		logicDetector_pre_set[i]->SetSensitiveDetector(sensDet);
	}
}
...
// Preliminary setup detector
void MyDetectorConstruction::ConstructPreSetupDetector(){
	std::string partical_name_list[] = {"mXD1", "mXD2", "mXD3", "mXU1", "mXU2", "mXU3",
	"XD1", "XD2", "XD3", "XU1", "XU2", "XU3",
	"mZD1", "mZD2", "mZD3", "mZU1", "mZU2", "mZU3",
	"ZD1", "ZD2", "ZD3", "ZU1", "ZU2", "ZU3"};
	
	int Size_of_names=sizeof(partical_name_list)/sizeof(std::string);
	for(int i=0;i<Size_of_names;i++){
		std::string name=partical_name_list[i];
		auto mesh=CADMesh::TessellatedMesh::FromSTL(name+".stl");
		auto Mesh=mesh->GetSolid();
		G4LogicalVolume* logicDetector_pre_ = new G4LogicalVolume(Mesh, scintillator, name+"logic");
		G4VPhysicalVolume* physDetector_pre_ = new G4PVPlacement(0, G4ThreeVector(0.*m, 0.*m, 0*m), logicDetector_pre_,name, logicWorld, false, 0, true);
		logicDetector_pre_set[i]=logicDetector_pre_;
	}
	//auto mesh=CADMesh::TessellatedMesh::FromSTL("ScintillatorOnlyASCII.stl");
	//auto Mesh=mesh->GetSolid();
	//logicDetector_pre = new G4LogicalVolume(Mesh, scintillator, "prelogicDetector");
	//physDetector_pre = new G4PVPlacement(0, G4ThreeVector(0.*m, 0.*m, 0*m), logicDetector_pre, "preDetector", logicWorld, false, 0, true);
	
}
// End Preliminary setup detector
